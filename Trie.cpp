...
ll mod_div(ll a, ll b, ll m) {a = a % m; b = b % m; return (mod_mul(a, mminvprime(b, m), m) + m) % m;}  //onliney for prime m
...
struct Node {
    vector<int> children;
    int strings_ending_here = 0;
    int strings_going_below = 0;
    Node() {
...
struct Trie{
    ...
    Trie(){
        ...
        sizeOfTrie++;
        // this creates a new node and
        // pushes at the back of the current vector
    }
    void add_string(string x) {
        ...
        for(char c : x){
            ...
            if(trie_tree[current_node].children[index] == -1) {
                  trie_tree[current_node].children[index] = sizeOfTrie;
                  trie_tree.emplace_back();
                  sizeOfTrie++;
            }
    ...
    void delete_unused_nodes(vector<pair<int, int>> nodes){
        for(pair<int, int> node : nodes){
            ...
            if(child.strings_going_below == 0){
                // for this node, the child at index node.second has not    
    ...
    void delete_string(string x){
        if(!search_string(x))
            return;
        int current_node = 0;
        vector<pair<int, int>> encounteredNodes;
        for(char c : x){
            int index = c - 'a';
            int parent = current_node;
            trie_tree[current_node].strings_going_below--;
            current_node = trie_tree[current_node].children[index];
            encounteredNodes.push_back({parent, index});
        }
        trie_tree[current_node].strings_going_below--;
        trie_tree[current_node].strings_ending_here--;
    }
...
void solve() {
    t.add_string("abcd");
    t.add_string("abcd");
    cout << t.search_string("abcd") << endl;
    cout << t.search_string("abc") << endl;
    
    t.add_string("abc");
    cout << t.search_string("abc") << endl;
    
    t.delete_string("abcd");
    cout << t.search_string("abcd") << endl;
